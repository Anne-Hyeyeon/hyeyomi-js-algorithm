...
- `최상위 원소(Top)` : 스택의 최상위 원소(마지막에 들어온 원소)를 확인하는 연산
- `Empty` : 스택이 비어 있는지 확인하는 연산 - true or false 리턴

### JS에서 스택을 구현하는 방법 - 배열 자료형
- 기본적으로 배열 자료형은, 다음의 두 가지 메서드를 제공한다.
- `push()` : 마지막 위치에 원소를 삽입, 시간 복잡도는 O(1)
- `pop()` : 마지막 위치에서 원소를 추출하며, 시간 복잡도는 O(1)
- 최상단 원소부터 출력하는 예시
```js
...
- ### 인접 리스트 - 그래프를 리스트로 표현함
- 노드마다 자신과 연결되어있는 노드를 리스트의 원소 형태로써 표현함.
- ![image](https://github.com/Anne-Hyeyeon/hyeyomi-js-algorithm/assets/93920435/8c0e99d0-5894-4173-b3aa-b3081f2df037)
- nxn짜리 매트릭스가 필요하지 않기 때문에 메모리를 효율적으로 쓸 수 있다.
...
![image](https://github.com/Anne-Hyeyeon/hyeyomi-js-algorithm/assets/93920435/6e711a3b-fbe9-47a5-aab0-c861ba06910e)
- 배열을 통해 어떤 노드와 연결되어 있는지 나타낼 수 있다. 이런 표현 방식을 인접 리스트 방식이라고 한다.
